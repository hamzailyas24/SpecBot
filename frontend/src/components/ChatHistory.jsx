...
function HistoryItem({ item, onContinue }) {
        ...
        <div className="px-3 pb-3 border-t border-border">
          <div className="mt-3 text-xs text-subtle leading-relaxed bg-surface rounded-lg p-3 prose-ai"
            dangerouslySetInnerHTML={{
              __html: item.response.slice(0, 400) + (item.response.length > 400 ? "…" : "")
            }} />
          <button
            onClick={() => onContinue(item)}
            className="mt-3 w-full flex items-center justify-center gap-2 py-2 rounded-xl
              text-xs font-medium border border-accent/30 text-accent
...
export default function ChatHistory({ trigger, onContinue }) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2 min-h-0">
        {loading
          ? [...Array(4)].map((_, i) => (
              <div key={i} className="h-14 rounded-xl shimmer-bg bg-elevated" />
            ))
          : history.length === 0
            ? (
              <div className="text-center py-10">
                <Clock size={26} className="text-border mx-auto mb-3" />
                <p className="text-subtle text-sm">No history yet</p>
              </div>
            )
            : history.map((item) => (
                <HistoryItem key={item.id} item={item} onContinue={onContinue} />
              ))
        }
      </div>
    </div>
